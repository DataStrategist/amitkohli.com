...
visNetwork(small_nodes, small_edges)
```
We are getting there! We could add the legend, but that might be clunky. For sure we should add what the actual indicator names are, because unless you work very closely with this data, these indicator numbers can be a bit abstract. These will be visible on mouseover to not clutter the UI.
...
```
I hate the legend... let's turn it off. Other than that, voila, the full network!
...
g$nodes <- g$nodes %>% 
  mutate(sdg = str_pad(sdg, width = 2,pad = "0"),
         group_name = paste0(sdg, " - ", group_name))

top_g <- g$edges %>% 
  left_join(g$nodes %>% select(from = id, from_group = group_name), by = "from") %>% 
  left_join(g$nodes %>% select(to = id, to_group = group_name), by = "to") %>% 
